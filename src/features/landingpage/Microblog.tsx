...
const getInstagramPosts = async () => {
  ...
  const options = {
    method: "GET",
    headers: {
      "X-RapidAPI-Key": "3ea13eb795mshdc23ce6925fc8e1p160395jsnf9a5f09e29e1",
    ...
    <div className="mt-12" id="project">
      <Container>
        <div className="flex items-center gap-4">
          <Image src={Blog.src} alt="Blog icon" width={72} height={72} />
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-8">
          {posts?.status !== "success" ? (
            ...
            posts.data.edges
              .slice(0, 3)
              .map((post, index) => (
                <InstagramPost key={index} postId={post.node.shortcode} />
              ))
          )}
        </div>
      </Container>
